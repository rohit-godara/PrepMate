...
import { MdEdit, MdSave, MdLogout, MdAdd, MdClose, MdPerson, MdEmail, MdPhone, MdSchool, MdWork, MdCode, MdBarChart } from "react-icons/md";
      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          <div className="flex items-start justify-between mb-6">
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-2">
                  <span className="bg-black text-white text-xs px-3 py-1 rounded-full">{user?.credits} credits</span>
                </div>
              </div>
            </div>
            <button
            ...
            >
              {editing ? <><MdSave size={16} /> {loading ? "Saving..." : "Save"}</> : <><MdEdit size={16} /> Edit Profile</>}
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Phone */}
            <Field icon={<MdPhone size={16} />} label="Phone" name="phone" value={form.phone} editing={editing} onChange={handleChange} placeholder="+91 XXXXX XXXXX" />

            {/* College */}
            <Field icon={<MdSchool size={16} />} label="College / University" name="college" value={form.college} editing={editing} onChange={handleChange} />
            ...
            {/* Target Role */}
            <Field icon={<MdWork size={16} />} label="Target Role" name="targetRole" value={form.targetRole} editing={editing} onChange={handleChange} placeholder="SDE-1, ML Engineer..." />
          ...
          {/* Skills */}
          <div className="mt-4">
            <label className="text-xs text-gray-500 font-medium mb-2 block flex items-center gap-1"><MdCode size={14} /> Skills</label>
          ...
          {!editing && (form.github || form.linkedin) && (
            <div className="flex gap-3 mt-4">
              {form.github && (
                <a href={form.github} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm text-gray-600 hover:text-black transition">
                  <FaGithub size={16} /> GitHub
                </a>
              )}
              {form.linkedin && (
                <a href={form.linkedin} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800 transition">
...
function Field({ icon, label, name, value, editing, onChange, placeholder }) {
  return (
    <div>
      <label className="text-xs text-gray-500 font-medium mb-1 flex items-center gap-1">{icon} {label}</label>
      {editing ? (
        <input
          name={name}
          value={value}
          ...
          className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-700 focus:outline-none focus:border-black transition"
        />
      ) : (
